...
const Register = ({ navigation }) => {
    ...
    const freeRegister = () => {
      ...
      let openImagePickerAsync = async () => {
        ...
        let blob = dataURItoBlob(pickerResult)
        let blobCreate = new FormData(document.forms[0])
        let imgTransition = blobCreate.append("File", blob)
      
          setImg( imgTransition );
      }

      const handleChangePhotoUrl = (event) => {
        event.preventDefault()
        const uploadTask = storage.ref(`msh-photo-user/${name}`).putString(img)
        uploadTask.on(
          "state_changed",
          snapshot => {},
